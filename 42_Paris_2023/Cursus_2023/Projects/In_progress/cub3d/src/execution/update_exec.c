...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/04/04 06:11:44 by mechard           #+#    #+#             */
/*   Updated: 2025/04/04 06:11:44 by mechard          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	update_translation_forward_backward(t_cub *cub, t_key *key)
{
	if (cub->move_forward)
	{
		if (cub->map[(int)(cub->posy)]
			...
			cub->posx = cub->posx + cub->dirx * key->ms;
		if (cub->map[(int)(cub->posy + cub->diry * key->ms)]
			[(int)cub->posx] != '1')
			cub->posy = cub->posy + cub->diry * key->ms;
	}
...
void	update_translation_strafe(t_cub *cub, t_key *key)
{
	if (cub->move_left)
	{
		...
		if (cub->map[(int)(cub->posy)]
			...
			cub->posx = cub->posx + key->strafex * key->ms;
		if (cub->map[(int)(cub->posy + key->strafey * key->ms)]
			[(int)cub->posx] != '1')
			cub->posy = cub->posy + key->strafey * key->ms;
	}
...
void	update_rotation(t_cub *cub, t_key *key)
{
	if (cub->pov_right)
	{
		...
		cub->planey = key->oldplanex
			* sin(key->rs) + cub->planey * cos(key->rs);
	}
