...
void	update_translation_forward_backward(t_cub *cub, t_key *key)
{
	if (cub->move_forward)
	{
		if (cub->map[(int)(cub->posy)][(int)((cub->posx + 0.2 * cub->dirx)
				+ cub->dirx * key->ms)] != '1')
			cub->posx += cub->dirx * key->ms;
		if (cub->map[(int)((cub->posy + 0.2 * cub->diry) + cub->diry
				* key->ms)][(int)cub->posx] != '1')
...
void	update_translation_strafe(t_cub *cub, t_key *key)
{
	if (cub->move_left)
	{
		...
		if (cub->map[(int)(cub->posy)][(int)((cub->posx + 0.2 * key->strafex)
				+ key->strafex * key->ms)] != '1')
			cub->posx += key->strafex * key->ms;
		if (cub->map[(int)((cub->posy + 0.2 * key->strafey) + key->strafey
				* key->ms)][(int)cub->posx] != '1')
...
void	update_rotation(t_cub *cub, t_key *key)
{
	...
	else if (cub->pov_left)
	{
		...
		cub->diry = key->olddirx * sin(-key->rs) + cub->diry * cos(-key->rs);
		key->oldplanex = cub->planex;
		cub->planex = cub->planex * cos(-key->rs) - cub->planey
			* sin(-key->rs);
		cub->planey = key->oldplanex * sin(-key->rs) + cub->planey
			* cos(-key->rs);
	}
...
int	update_loop(t_cub *cub)
{
	...
	if (cub->move_forward || cub->move_backward || cub->move_left
		|| cub->move_right)
