...
void	write_status(char *status_message, t_philo *philosopher)
{
	...
	if (current_time >= 0 && current_time <= 2147483647
		...
		printf("%ld %d %s", current_time, philosopher->id, status_message);
}
...
void	ft_secure(t_philo *philosopher, int i)
{
	if ((philosopher->id + i) % 2 == 1)
